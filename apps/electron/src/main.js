...
ipcMain.handle("desktop:save-pomodoro-bg", async (_event, dataUrl) => {
  try {
    fs.writeFileSync(pomodoroBgPath(), dataUrl, "utf8");
    return true;
  } catch (e) {
    return false;
  }
...
ipcMain.handle("desktop:load-pomodoro-bg", async () => {
  try {
    if (fs.existsSync(pomodoroBgPath())) {
      return fs.readFileSync(pomodoroBgPath(), "utf8");
    }
  } catch (e) { /* ignore */ }
  return null;
});
