...
tar -czf core/deploy/deploy.tar.gz core/backend/ core/frontend/ core/requirements.txt 2>$null
Write-Host "  打包完成: deploy/deploy.tar.gz"

if (-not $SkipUpload) {
    # 2. 上传
    Write-Host "[2/4] 上传到云端..." -ForegroundColor Yellow
    scp core/deploy/deploy.tar.gz ${server}:/home/dayorder/
    Write-Host "  上传完成"
}
...
# 清理
Remove-Item core/deploy/deploy.tar.gz -Force -ErrorAction SilentlyContinue
